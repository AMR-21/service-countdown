...
export function useClockMode() {
	const [clockMode, setClockMode] = useQueryState(
		storageKey,
		parseAsStringLiteral(CLOCK_MODES).withDefault("days"),
	);
	...
	useEffect(() => {
		...
		const isStoredValid = CLOCK_MODES.includes(
			storedClockMode as (typeof CLOCK_MODES)[number],
		);
